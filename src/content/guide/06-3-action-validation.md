...
An Action existing and an Action being allowed to run right now are different questions. This subchapter covers the two-layer system that answers the second one, and wires the answer into menus, toolbars, and the responder chain without ever asking a view to make the decision.
