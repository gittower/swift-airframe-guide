...
The pattern instead is the same one from <a href="/guide/05-model-layer">Chapter 5</a>'s "scoped state" case: a parent view controller owns a small `@Observable` navigation state describing what's selected and what's showing. Children never see each other — they read plain input properties set by the parent, and own whatever loading state they need themselves.
